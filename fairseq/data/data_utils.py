...
def compute_mask_indices(
    ...
    Args:
        shape: 计算掩码的形状。大小应为2，其中第一个元素是批量大小，第二个元素是时间步长
        padding_mask: 可选的与形状相同大小的填充口罩，可防止遮挡填充的元素
        mask_prob: 每个令牌被选为要屏蔽的跨度的开始的概率。这将乘以时间步长除以掩模跨度的长度，以掩盖所有元素中大约这个百分比。但是，由于重叠，实际数字将较小（除非no_apload为True）
    ...
    """
    bsz, all_sz = shape
    mask = np.full((bsz, all_sz), False) # 全是false
    if num_mask_ver == 1: # 2
        all_num_mask = int(
    ...
    mask_idcs = []
    for i in range(bsz): # 8
        if seed is not None and epoch is not None and indices is not None:
            seed_i = int(hash((seed, epoch, indices[i].item())) % 1e6)
        else:
            seed_i = None
        rng = np.random.default_rng(seed_i)
        if padding_mask is not None: # wsy：enter
            sz = all_sz - padding_mask[i].long().sum().item() 
            assert sz >= 0, sz
        else: # wsy：enter，这里也进入了
            sz = all_sz
        if num_mask_ver == 1:
        ...
        #--------wsy fix-----------------------------------------------------------------------------
        mask_idc = np.unique(mask_idc[mask_idc < sz]) # sz=1162 
        # np.unique() 函数 去除其中重复的元素 ，并按元素 由小到大 返回一个新的无元素重复的元组或者列表。

        step1=mask_idc[mask_idc < sz]
        step2=np.unique(step1)
        if len(step2) >=sz:
            a=1
        #-------------------------------------------------------------------------
