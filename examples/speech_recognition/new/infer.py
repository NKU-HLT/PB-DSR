...
class InferenceProcessor:
    ...
    def __init__(self, cfg: InferConfig) -> None:
        ...
        if "adapter" in ckpt_obj: # wsy： 此处没有进入
        ...
        self.task.load_dataset(
            ...
            task_cfg=saved_cfg.task, # hubert_pretraining
        )
...
def hydra_main(cfg: InferConfig) -> Union[float, Tuple[float, Optional[float]]]:
    #----wsy add----------------------------------------------------------------------------
    cfg=OmegaConf.load(os.getenv("WORK_DIR")+"/wsy/config/config.yaml")
    #--------------------------------------------------------------------------------------
    container = OmegaConf.to_container(cfg, resolve=True, enum_to_str=True)
    #----wsy add----------------------------------------------------------------------------------
    container["common"]["user_dir"]=os.getenv("WORK_DIR")+"/examples/data2vec"
    #---------------------------------------------------------------------------------------------
...
def cli_main() -> None:
    try:
        ...
        cfg_name = get_args().config_name or "infer_viterbi"
        #----------------------------------------------------
    except ImportError:
